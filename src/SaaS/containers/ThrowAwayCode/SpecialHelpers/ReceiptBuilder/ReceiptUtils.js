...
const storeInfoHeader = (confirmationNum, authorizationNum, seriesNum, nit)=>{
  ...
  const TIMESTAMP = "*Fecha y hora de emisión:*%0A" + TIMESTAMP_GENERATOR().split('&').join(' ')+"%0A";
...
const customerInfoHandler = (name,consumption, total, nit, address, paymentMethod)=>{
    ...
    const PURCHASE_STRING = ", su consumo por *" + consumption +"* con modalidad de pago *"+paymentMethod+  "* en " + STORENAME+ " es un total de *Qt.z" + total+ '*.%20';
...
const purchaseJson = (isConsumptionGood, product, total) => {
    var processing = {};
    const typeOfReceipt = isConsumptionGood ? "B" : "S";
     processing[product] = {
      "precio": 1,
            "unidades": 1,
            "PoS": typeOfReceipt,
...
export const callAPI4Receipt = ( receiptUtils, taxDetail) => {
    const [ name,consumption, total, nit, address, phoneNum, paymentMethod]= receiptUtils;
    const [authorizationNum,seriesNum, confirmationNum] = taxDetail;
    const CUSTOMER_INFO = customerInfoHandler( name,consumption, total, nit, address, paymentMethod)
